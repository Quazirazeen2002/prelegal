...
export default function MyDocumentsModal({ onClose, onLoad }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-surface p-7 shadow-2xl shadow-black/40">
        ...
        <ul className="space-y-2">
          {documents?.map((doc) => (
            <li
              key={doc.id}
              className="flex items-center justify-between gap-3 rounded-xl border border-white/10 p-4 transition-colors hover:border-brand-green/30 hover:bg-brand-green/5"
            >
              <div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => onLoad(doc)}
                  className="rounded-full bg-brand-green px-3.5 py-1.5 text-xs font-bold text-background shadow-sm transition-colors hover:bg-brand-green-dark"
                >
                  Load
                </button>
                <button
                  ...
                  className="rounded-full border border-white/15 px-3.5 py-1.5 text-xs font-medium text-ink-muted transition-colors hover:bg-white/10"
                >
                  Delete
                </button>
              </div>
