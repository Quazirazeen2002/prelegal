...
const AnalysisReportDownloadButton = dynamic(() => import("./AnalysisReportDownloadButton"), {
  ssr: false,
  loading: () => (
    <span className="inline-flex items-center justify-center rounded-full bg-white/10 px-5 py-2.5 text-sm font-medium text-ink-muted">
      Loading…
    </span>
  ),
});
...
export default function ExportReportView({ upload }: { upload: UploadedDocumentDetail | null }) {
  return (
    <AnalysisStateGate upload={upload}>
      {(doc) => (
        <div className="max-w-xl rounded-2xl border border-white/10 bg-surface p-7 shadow-xl shadow-black/20">
